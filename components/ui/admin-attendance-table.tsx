...
export default function AdminAttendanceTable({ tableDetails }: TableProps) {
  ...
  const [filterBy, setFilterBy] = useState<"all" | "name" | "employeeId">(
    "all"
  );
  ...
  useEffect(() => {
    const filtered = tableDetails.filter((item) => {
      ...
      if (filterBy === "name") {
        ...
        return item.employeeId.toString().toLowerCase().includes(searchLower);
      } else {
        return (
      ...
      <div className="w-full bg-white dark:bg-gray-900 p-4 sm:p-6">
        ...
        {filteredData.length === 0 && searchTerm.length > 0 && (
          <div className="flex flex-col items-center justify-center py-20 px-4">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 text-center max-w-md mb-6">
              Looks like there are no attendance here on the HR Mini. Click the
